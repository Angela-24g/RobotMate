...
create table RM_BBS (
    ...
    title               VARCHAR(1000)   NULL                                COMMENT '게시판 제목',
    contents            TEXT            NULL                                COMMENT '게시판 내용',
    view_cnt            INTEGER         NOT NULL        DEFAULT 0           COMMENT '게시판 조회수',
    pbc_yn              CHAR            NOT NULL        DEFAULT 'Y'         COMMENT '게시판 공개여부(Y:공개,N:비공개)',
    bbs_type            VARCHAR(50)     NOT NULL        DEFAULT 'FREE'      COMMENT '게시판 종류(FREE:자유게시판, NOTICE:공지사항)',
    user_id             BIGINT          NULL                                COMMENT '게시판 작성자 식별자',
    del_yn              CHAR            NOT NULL        DEFAULT 'N'         COMMENT '게시판 삭제여부(Y:삭제,N:미삭제)',
    create_dt           DATETIME        NOT NULL        DEFAULT NOW()       COMMENT '게시판 생성일',
    mod_dt              DATETIME        NOT NULL        DEFAULT NOW()       COMMENT '게시판 수정일',
    PRIMARY KEY (id)
);
...
create table RM_FILE (
    ...
    type                VARCHAR(50)     NOT NULL                            COMMENT '파일 타입(BBS:게시판,REQ:문의사항,GALLERY:갤러리)',
